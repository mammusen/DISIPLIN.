...
- **I dag** — huk av oppgaver etter hvert som du gjør dem.
...
- **Streak** 🔥 — teller sammenhengende dager der du både planla kvelden før
  *og* gjennomførte kveldsrefleksjonen. Brytes hvis du planlegger for sent
  eller hopper over en dag.
...
Innlogging fungerer som på Netflix: du møtes av en profilvelger med seks
faste profiler — **Amalie, Jo Kristian, Marius, Per Jørgen, Pål Andre,
Torill Anita**. Første gang noen trykker på sin profil lager de en
4-sifret kode (skrives inn to ganger), og den koden bruker de for å låse
opp profilen sin ved senere besøk. Ingen brukernavn eller passord å holde
styr på.
...
profil oppe til høyre (Mørk, Girly, Hav, Skog, Solnedgang).

Vil dere endre hvem de seks profilene er? Rediger `PROFILE_NAMES`-listen
øverst i `lib/db.js` og deploy på nytt. Endrer du et navn på en profil som
allerede har fått en kode, blir ikke den koden rørt — kun ubrukte profiler
får nytt navn automatisk ved neste oppstart.

**Glemt kode?** Det finnes ingen "glemt kode"-knapp i appen. Går det galt
må dere nullstille koden manuelt i databasen (se under).
...
**Nullstille en glemt kode manuelt (Neon):** gå til prosjektet ditt på
[neon.com](https://neon.com) → **SQL Editor**, og kjør (bytt ut `X` med
plassnummeret til profilen — 1 = Amalie, 2 = Jo Kristian, 3 = Marius,
4 = Per Jørgen, 5 = Pål Andre, 6 = Torill Anita):

```sql
UPDATE profiles SET pin_hash = NULL WHERE slot = X;
```

Da blir profilen "ledig" igjen, og personen kan trykke på den og lage en
ny kode — akkurat som første gang. Selve dataene (XP, streak, oppgaver)
rører dette ikke.
...
- `lib/db.js` — lagring: lokal fil som standard, Postgres når
  `DATABASE_URL` er satt (brukes automatisk ved sky-hosting). De seks
  faste profilene (`PROFILE_NAMES`) og hver persons dagsplan-data.
